...
*   **알고리즘 트레이딩 (Advanced Logic)**:
    *   **메인 전략 (Trend)**: 변동성 돌파 (Volatility Breakout) + 이동평균(MA5) 추세 필터.
        *   `현재가 > (시가 + 전일 변동폭 * K)` 이면서 `현재가 > 5일 이동평균` 일 때 매수.
        *   **동적 K (Dynamic K)**: 최근 20일간의 노이즈 비율(Noise Ratio) 평균을 사용하여 K값 자동 최적화.
    *   **서브 전략 (Mean Reversion)**: 볼린저 밴드 하단 돌파 + 저점 매수.
        *   `현재가 < 볼린저 밴드 하단` 이고 `RSI(14) < 30` 일 때 과매도 구간으로 판단하여 소액 매수 시도 (Mean Reversion).
    *   **자금 관리 (Risk Management)**:
        *   **변동성 타겟팅 (Volatility Targeting)**: ATR(Average True Range)을 활용하여 변동성이 큰 종목은 비중을 줄이고, 변동성이 작은 종목은 더 실음으로써 리스크를 자산 대비 2% 수준으로 일정하게 유지.
        *   **트레일링 스탑 (Trailing Stop)**: 진입 후 최고가 대비 3% 하락 시 즉시 매도하여 수익을 확정하고 손실을 제한.
    *   **안전 장치**:
        *   **Pump & Dump 감지**: 순간 체결 거래량이 최근 10분 평균의 10배 이상 폭증할 경우 매수 보류 (세력에 의한 인위적 펌핑 방지).
*   **웹 대시보드**:
...
2.  **봇 시작**
    ...
    *   대시보드 접속: `http://localhost:5555`

## 📊 대시보드

실행 후 브라우저에서 `http://localhost:5555` (또는 서버 IP)에 접속하면 다음 정보를 확인할 수 있습니다:
